...
### 类
- 定义
   1. C++ 中使用一个类（class）来定义一个数据类型（data structure）。
   2. 一个类定义了一个类型，以及关联的一系列操作。
   3. C++ 语言设计的焦点之一就是想让使用定义使用上像内置类型一样自然的类类型（class type）
`#include "ClassName.h"`
注意使用自定义头文件时，需要使用双引号（""）包裹。而非尖括号。

#### 成员函数
成员函数是定义在类中的函数，也被称为方法（method）。
点运算符（.），点运算符只作用于类类型的对象，左侧是一个类类型对象，右侧是该对象的成员名，可以为函数也可以为属性。
当使用点运算符访问一个成员函数时，通常使用调运运算符（()）且为一个括号，里面放着实参（argument）列表（可以为空）。
